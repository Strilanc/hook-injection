from typing import Union, List, Tuple, Any, Optional, Dict, Callable, Literal

import stim

from hookinj.gen._chunk import Chunk
from hookinj.gen._flow import PauliString, Flow
from hookinj.gen._builder import MeasurementTracker, Builder, AtLayer
from hookinj.gen._patch import Patch
...
def reindexed_circuit(
    circuit: stim.Circuit,
    *,
    old2new: Dict[int, int],
    discard_qubit_coords: bool = False,
    out: Optional[stim.Circuit] = None,
) -> stim.Circuit:
    if out is None:
        out = stim.Circuit()
    for inst in circuit:
        if isinstance(inst, stim.CircuitRepeatBlock):
            out.append(stim.CircuitRepeatBlock(
                ...
                body=reindexed_circuit(
                    circuit=inst.body_copy(),
                    old2new=old2new,
                )
            ))
        elif isinstance(inst, stim.CircuitInstruction):
            if discard_qubit_coords and inst.name == "QUBIT_COORDS":
                continue
            targets = []
            for t in inst.targets_copy():
                if t.is_qubit_target:
                    targets.append(old2new[t.value])
                elif t.is_x_target:
                    targets.append(stim.target_x(old2new[t.value]))
                elif t.is_y_target:
                    targets.append(stim.target_y(old2new[t.value]))
                elif t.is_z_target:
                    targets.append(stim.target_z(old2new[t.value]))
                elif t.is_combiner:
                    targets.append(t)
                else:
...
def verify_circuit_has_all_possible_detectors(circuit: stim.Circuit):
    ...
    # last_obs_sightings = {}
    # tick = 0
    # def pre_run_block(block: stim.Circuit, reps: int):
    #     nonlocal num_determined_measurements, tick, num_detectors_seen
    #     for _ in range(reps):
    #         for inst in block:
    #             if isinstance(inst, stim.CircuitRepeatBlock):
    #                 pre_run_block(inst.body_copy(), inst.repeat_count)
    #             elif inst.name == 'OBSERVABLE_INCLUDE':
    #                 last_obs_sightings[int(inst.gate_args_copy()[0])] = tick
    #             elif inst.name == 'TICK':
    #                 tick += 1
    # pre_run_block(circuit, 1)
        ...
        for _ in range(reps):
            for inst in block:
                if isinstance(inst, stim.CircuitRepeatBlock):
                    ...
                    tick += 1
                    # num_observables_done = sum(v <= tick for v in last_obs_sightings.values())
                    # if num_detectors_seen + num_observables_done != num_determined_measurements:
                    #     raise ValueError(f"at {tick=} found {num_detectors_seen=} + {num_observables_done=} != {num_determined_measurements=}")
